...
const BASE_IMAGE_URL = `http://${IP}:333/chapter/`; // Change to 10.0.2.2 for Android emulator, localhost for iOS simulator
...
export default function ChapterScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (manga) {
      manga.image.forEach((fileName, idx) => {
        const uri = `http://${IP}:333/chapter/${fileName}`;
        Image.getSize(
          uri,
          (width, height) =>
  ...
  useEffect(() => {
    if (manga && manga._id) {
      axios.get(`http://${IP}:333/api/comment/chapter/${manga._id}`)
